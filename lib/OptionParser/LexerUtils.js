...
export function CharacterStream(fullString, pos = 0) {
  return {
...
export function concat(type, matcher) {
  return function(previousTokens, charStream) {
    var match = matcher(previousTokens, charStream);

    if (match) {
      var joinedToken = match.tokens.map((t) => t.token).join("");
...
export function notFollowedBy(mustMatch, mustNotMatch) {
  return function(previousTokens, charStream) {
    ...
    var nextMatch = mustNotMatch([...previousTokens, ...match.tokens], match.newCharacterStream);
